<template>
  <div>
    <p class="muted" style="margin-bottom: 24px;">前後端分頁參數與回應結構的統一規範。</p>

    <!-- 查詢時 -->
    <div style="margin-bottom: 48px;">
      <h3 style="font-size: 1.25rem; margin-bottom: 20px; color: var(--text);">🔍 查詢時（前端 → 後端）</h3>
      <div class="grid">
        <article class="card">
          <h4 style="margin-bottom: 12px; font-size: 1rem; color: #059669;">前端送出參數</h4>
  ...
  ...其他查詢參數
    ...
    <!-- 回應時 -->
    <div style="margin-bottom: 48px;">
      <h3 style="font-size: 1.25rem; margin-bottom: 20px; color: var(--text);">📦 回應時（後端 → 前端）</h3>
      <div class="grid">
        <article class="card">
          <h4 style="margin-bottom: 12px; font-size: 1rem; color: #2563eb;">後端回傳結構</h4>
  ...
  "offset": 0          // 位移量（選填）
        ...
        <article class="card">
          <h4 style="margin-bottom: 12px; font-size: 1rem; color: #059669;">前端處理方式</h4>
          <p style="margin-bottom: 12px; color: var(--muted);">使用 <code style="background: #f3f4f6; padding: 2px 6px; border-radius: 4px;">extractPaginationContent</code> 自動解析：</p>
          <ul>
            <li>提取 <code style="background: #f3f4f6; padding: 2px 6px; border-radius: 4px;">content</code> 資料陣列</li>
            <li>自動計算並附加 <code style="background: #f3f4f6; padding: 2px 6px; border-radius: 4px;">$$RowNumber</code>（序號）</li>
            <li>自動附加 <code style="background: #f3f4f6; padding: 2px 6px; border-radius: 4px;">$$RowIndex</code>（索引）</li>
            <li>更新分頁控制器狀態</li>
          </ul>
        </article>
      </div>
    ...
    <n-collapse style="margin-bottom: 24px;">
      <n-collapse-item name="component">
        <template #header>
          <span style="font-size: 1.1rem; font-weight: 600;">📄 分頁元件完整程式碼</span>
        </template>
        <pre style="margin: 0;">import { toValue } from '@vueuse/core'
  ...
  return {
    ...
    <n-collapse>
      <n-collapse-item name="example">
        <template #header>
          <span style="font-size: 1.1rem; font-weight: 600;">💡 使用範例</span>
        </template>
        <pre style="margin: 0;">// 1. 初始化分頁（可自訂每頁筆數）
