...
mutationCache: new MutationCache({
  ...
  onError: (error) => {
    ...
    if (error?.displayMessage) {
      ...
      <article class="card" style="background: #f0fdf4; border-color: #86efac;">
        ...
        <ol style="margin: 0; padding-left: 20px; line-height: 2; color: #166534;">
          ...
          <li>後端處理完畢後回傳 response（包含 message、success、data）</li>
    ...
    <!-- Message 格式規範 -->
    <div style="margin-top: 48px;">
      <h3 style="font-size: 1.25rem; margin-bottom: 20px; color: var(--text);">💬 Message 格式規範</h3>
      
      <div class="callout" style="margin-bottom: 24px; background: #eff6ff; border-color: #93c5fd;">
        <p style="margin-bottom: 12px;">
          <strong>⚠️ 重要原則</strong>
        </p>
        <p style="margin: 0; line-height: 1.8;">
          後端在制訂 message 時，<strong>必須符合情境回復相應的中文內容</strong>。使用者不需要看到工程師才看得懂的程式碼或技術訊息，只需要<strong>簡單明瞭的中文說明</strong>。
        </p>
      </div>

      <div class="grid">
        <article class="card" style="background: #fef2f2; border-color: #fca5a5;">
          <h4 style="margin-bottom: 16px; color: #991b1b;">❌ 錯誤範例</h4>
          <div style="margin-bottom: 16px;">
            <p style="color: #7f1d1d; margin-bottom: 8px; font-weight: 600;">1. 包含程式碼或技術訊息</p>
            <pre>{
  "success": false,
  "message": "unexpected: 會員資格不符"
}</pre>
            <div style="margin-top: 8px; padding: 10px; background: #fee2e2; border-radius: 4px;">
          ...
          <div style="margin-bottom: 16px;">
            <p style="color: #7f1d1d; margin-bottom: 8px; font-weight: 600;">2. 包含陣列格式</p>
            <pre>{
  "success": false,
  "message": "[會員資格不符]"
}</pre>
          ...
          <div>
            <p style="color: #7f1d1d; margin-bottom: 8px; font-weight: 600;">3. 包含錯誤代碼或內部訊息</p>
            <pre>{
  "success": false,
  "message": "Error Code 4001: 使用者資格驗證失敗"
        ...
        <article class="card" style="background: #f0fdf4; border-color: #86efac;">
          <h4 style="margin-bottom: 16px; color: #15803d;">✓ 正確範例</h4>
          <div style="margin-bottom: 16px;">
            <p style="color: #166534; margin-bottom: 8px; font-weight: 600;">1. 清楚明確的中文說明</p>
            <pre>{
  "success": false,
  "message": "會員資格不符"
}</pre>
            <div style="margin-top: 8px; padding: 10px; background: #dcfce7; border-radius: 4px;">
          ...
          <div style="margin-bottom: 16px;">
            <p style="color: #166534; margin-bottom: 8px; font-weight: 600;">2. 提供解決方向的說明</p>
            <pre>{
  "success": false,
  "message": "手機號碼格式錯誤，請輸入10位數字"
          ...
          <div>
            <p style="color: #166534; margin-bottom: 8px; font-weight: 600;">3. 符合情境的友善訊息</p>
            <pre>{
  "success": false,
  "message": "檔案大小超過限制，請上傳5MB以下的檔案"
      ...
      <div class="callout" style="margin-top: 24px; background: #f0f9ff; border-color: #7dd3fc;">
        <p style="color: #0c4a6e; margin-bottom: 12px;">
          <strong>💡 撰寫 Message 的原則</strong>
        </p>
        <ul style="margin: 0; padding-left: 20px; line-height: 2; color: #0c4a6e;">
          <li><strong>使用純中文：</strong>避免英文技術用語、程式碼或符號</li>
          <li><strong>說明問題：</strong>清楚告知使用者發生了什麼事</li>
          <li><strong>提供方向：</strong>盡可能告訴使用者如何解決或修正</li>
          <li><strong>保持簡潔：</strong>一句話說清楚，不要冗長或過於技術性</li>
          <li><strong>站在使用者角度：</strong>確保一般使用者能夠理解訊息內容</li>
        </ul>
      </div>
    </div>
    ...
    <!-- 例外狀況：什麼時候不用回傳 data? -->
    <div style="margin-top: 48px;">
      <h3 style="font-size: 1.25rem; margin-bottom: 20px; color: var(--text);">🔄 例外狀況：什麼時候不用回傳 data?</h3>
      ...
      <div class="callout" style="margin-bottom: 24px; background: #fef3c7; border-color: #fbbf24;">
        ...
        <p style="color: #92400e; margin: 0; line-height: 1.8;">
          針對「上傳檔案」或「新增/修改資料」的 API，如果操作完成後<strong>不需要進一步的行為</strong>，則成功時可以<strong>不回傳 data 參數</strong>，僅回傳 success 和 message 即可。
        </p>
      </div>

      <div class="grid" style="margin-bottom: 24px;">
        <article class="card" style="background: #f0fdf4; border-color: #86efac;">
          <h4 style="margin-bottom: 16px; color: #15803d;">✓ 成功時：不需要 data</h4>
          <p style="color: #166534; margin-bottom: 12px; font-size: 0.95rem;">
            上傳成功後如果沒有需要前端進一步處理的資料，直接回傳操作結果即可
          </p>
          <pre>{
  "success": true,
  "message": "上傳成功"
}</pre>
          <div style="margin-top: 12px; padding: 12px; background: #dcfce7; border-radius: 6px;">
            <p style="color: #166534; margin: 0; font-size: 0.9rem;">
              <strong>適用情境：</strong>單純的上傳、新增或修改操作，只單純告知使用者操作結果即可
            </p>
          </div>
        </article>

        <article class="card" style="background: #fef2f2; border-color: #fca5a5;">
          <h4 style="margin-bottom: 16px; color: #991b1b;">✗ 失敗時：不需要 data</h4>
          <p style="color: #991b1b; margin-bottom: 12px; font-size: 0.95rem;">
            操作失敗時回傳明確的錯誤訊息，讓使用者了解失敗原因
          </p>
          <pre>{
  "success": false,
  "message": "XXX並未符合資格"
...

{
  ...
  "message": "XXX並未依照指定格式輸入"
          ...
          <div style="margin-top: 12px; padding: 12px; background: #fee2e2; border-radius: 6px;">
            <p style="color: #991b1b; margin: 0; font-size: 0.9rem;">
              <strong>注意：</strong>message 需要清楚說明失敗原因，讓使用者知道如何修正
            </p>
          </div>
        </article>
      </div>
      ...
      <article class="card" style="background: #eff6ff; border-color: #93c5fd;">
        <h4 style="margin-bottom: 16px; color: #1e40af;">📦 什麼時候需要回傳 data？</h4>
        <p style="color: #1e3a8a; margin-bottom: 12px;">
          如果上傳或新增修改成功後，<strong>前端需要進一步的行為</strong>，則必須回傳 data 參數：
        </p>
        <ul style="margin: 0; padding-left: 20px; line-height: 2; color: #1e3a8a;">
          <li>需要顯示剛上傳檔案的資訊（如檔案 ID、檔案路徑等）</li>
          <li>需要導向至新建立的資料詳細頁（需要資料 ID）</li>
          <li>需要更新畫面上的其他資訊（如更新後的完整資料）</li>
          <li>需要下載或預覽剛上傳的檔案</li>
        </ul>
    ...
    "uploadTime": "2026-03-19 10:30:00"
      ...
      <div class="callout" style="margin-top: 24px; background: #f0f9ff; border-color: #7dd3fc;">
        <p style="color: #0c4a6e; margin: 0;">
          <strong>💡 設計建議：</strong>在設計 API 時，先確認前端在操作成功後是否需要額外的資料。如果只需要顯示「成功」訊息並關閉彈窗或返回列表頁，則不需要回傳 data，可以簡化 response 結構。
        </p>
      </div>
    </div>
        ...
        <table>
          ...
          <tbody>
            ...
            <tr>
              ...
              <td><strong>找不到API (Not Found)</strong></td>
              <td>API 路徑不存在</td>
            </tr>
            <tr>
            ...
            <tr>
              <td><strong style="color: #dc2626; font-size: 1.05rem;">422</strong></td>
              <td><strong>無法處理的實體 (Unprocessable Entity)</strong></td>
              <td>前端請求(request)格式正確，但因語義錯誤無法處理，例如資料驗證失敗</td>
            </tr>
            <tr>
